...
public class WaterYaoOpenRunable extends SerialControlRunable {

	private int id;
	private int source = -1;

	public WaterYaoOpenRunable(int source) {
	...
	public int onExecute(Handler handler, int currId) {
		...
		while (count < 10) {
			...
			sendSerialMsg(SerialCmd.CMD_WATER_YAO_OPEN);
		...
		if (count < 10) {
			...
			RunningData.getInstance().setYaoState(true);
	...
	@Override
	public int getType() {
		return Constants.MOTOR_CONTROL_TYPE_YAO_OPEN;
	}
